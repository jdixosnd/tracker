  ...
  const fetchData = useCallback(async (showLoading = true) => {
    ...
    try {
      ...
      const formattedData = parsedData.table.rows.map(row => {
          if (!row.c[0] || !row.c[1] || row.c[5] === null || !row.c[0].v) return null;
          const dateMatch = row.c[0].v.match(/Date\((\d+),(\d+),(\d+)\)/);
          const timeMatch = row.c[1].v.match(/Date\(.*?,(\d+),(\d+),(\d+)\)/);
      ...
      console.error(e);
      setError('Could not load feeding data. Please check your connection or the sheet URL.');
    } finally {
      ...
      <>
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6 mb-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <InsightCard title="Most Recent Feed" value={data.length > 0 ? data[0].date.toLocaleTimeString('en-US', {hour: '2-digit', minute:'2-digit'}) : 'N/A'} icon={<ClockIcon className="w-7 h-7 text-indigo-500"/>} />
            </div>
        </div>
